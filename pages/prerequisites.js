...
function renderPrerequisitesPage() {
  ...
  const commandDeck = createCardContainer(`
    ...
    <div class="prereq-command-row">
      ...
      <div class="prereq-command-right">
        ...
        <label id="prereq-debug-toggle-label" class="vpnsetup-debug-toggle" for="prereq-debug-toggle-input" style="display:none;">
          <input id="prereq-debug-toggle-input" type="checkbox">
          <span>Debug: Simulate multiple computers</span>
        </label>
      </div>
    </div>
  ...
  let debugSimEnabled = false;
  let debugVisibilityTimer = null;
  ...
  function showComputerPickerModal(configs) {
    return new Promise((resolve) => {
      const overlay = document.createElement('div');
      overlay.id = 'prereq-computer-picker-overlay';
      overlay.className = 'vpnsetup-modal-overlay';
      overlay.setAttribute('role', 'presentation');

      overlay.innerHTML = `
        <div
          id="prereq-computer-picker-modal"
          class="vpnsetup-remove-modal"
          role="dialog"
          aria-modal="true"
          aria-labelledby="prereq-computer-picker-title"
          aria-describedby="prereq-computer-picker-desc"
          tabindex="-1"
          style="max-width:480px;"
        >
          <div class="vpnsetup-remove-modal-header">
            <span class="material-icons vpnsetup-remove-modal-icon" style="color:var(--rewst-teal);">computer</span>
            <h4 id="prereq-computer-picker-title" class="vpnsetup-remove-modal-title">Select Your Computer</h4>
          </div>
          <p id="prereq-computer-picker-desc" class="vpnsetup-remove-modal-copy">
            Multiple computers were found for your account. Select the one you're setting up VPN on.
          </p>
          <div id="prereq-computer-picker-list" style="display:flex;flex-direction:column;gap:8px;margin-bottom:16px;">
            ${configs.map((config, i) => `
              <button
                class="btn-secondary prereq-computer-picker-item"
                data-index="${i}"
          ...
          <div class="vpnsetup-remove-modal-actions">
            <button id="prereq-computer-picker-cancel" class="btn-secondary" type="button">Cancel</button>
          </div>
        </div>
      `;
      ...
      const modal = overlay.querySelector('#prereq-computer-picker-modal');
      ...
      overlay.querySelectorAll('.prereq-computer-picker-item').forEach((btn) => {
        btn.addEventListener('click', () => {
          const index = parseInt(btn.dataset.index, 10);
          cleanup(configs[index] || null);
        });
      });

      overlay.querySelector('#prereq-computer-picker-cancel')?.addEventListener('click', () => cleanup(null));
  ...
  async function runCwmConfigurationCheck(options = {}) {
    ...
    try {
      ...
      if (debugSimEnabled) {
        const fakeConfigs = [
          { name: 'DESKTOP-DEBUG01', id: 'debug-id-1', deviceIdentifier: 'debug-cwa-1' },
          { name: 'LAPTOP-DEBUG02', id: 'debug-id-2', deviceIdentifier: 'debug-cwa-2' },
          { name: 'WORKSTATION-DEBUG03', id: 'debug-id-3', deviceIdentifier: 'debug-cwa-3' }
        ];
        setCheckLoading(cwmCheckItem, 'CWM Configuration', 'Debug simulation active — showing computer picker...');
        const picked = await showComputerPickerModal(fakeConfigs);
        if (picked) {
          checkStates.cwm_config = true;
          checkResultDetails.cwm_config = `Debug simulation — selected: ${picked.name}`;
          renderCheckResult(cwmCheckItem, true, 'CWM Configuration', checkResultDetails.cwm_config);
          selectedConfig = picked;
          window.selectedConfig = picked;
        } else {
          checkStates.cwm_config = false;
          checkResultDetails.cwm_config = 'Debug simulation — no computer selected';
          renderCheckResult(
            cwmCheckItem,
            false,
            'CWM Configuration',
        ...
        updateButtonState();
        // Halt pipeline — do not proceed to computer online checks with debug data.
        return;
      }
      ...
      if (!attemptResult.ok || validConfigs.length === 0) {
        ...
        const picked = await showComputerPickerModal(validConfigs);
        if (picked) {
          selectedConfig = picked;
          window.selectedConfig = selectedConfig;
          try {
          ...
          checkStates.cwm_config = false;
          checkResultDetails.cwm_config = 'No computer selected';
          renderCheckResult(
            cwmCheckItem,
            false,
            'CWM Configuration',
  ...
  async function resolveCwmConfigurations(result, operationName = 'User prerequisite: CWM Configuration') {
    ...
    const getRawConfigs = (inputResult) => {
      ...
      if (rawValue && typeof rawValue === 'object') {
        // Single config object returned directly (not wrapped in an array or keyed dict).
        if (rawValue.name && rawValue.id && rawValue.deviceIdentifier) {
          return [rawValue];
        }
  ...
  // ---- Debug toggle wiring ----
  const savedDebugSim = getStoredJson(PREREQ_DEBUG_SIM_KEY);
  debugSimEnabled = !!savedDebugSim?.enabled;

  const debugToggleLabel = commandDeck.querySelector('#prereq-debug-toggle-label');
  const debugToggleInput = commandDeck.querySelector('#prereq-debug-toggle-input');

  if (debugToggleInput) {
    debugToggleInput.checked = debugSimEnabled;
    debugToggleInput.addEventListener('change', (event) => {
      debugSimEnabled = !!event.target.checked;
      setStoredJson(PREREQ_DEBUG_SIM_KEY, { enabled: debugSimEnabled });
      if (debugSimEnabled) {
        RewstDOM.showWarning('Debug simulation enabled. CWM check will show a fake computer picker and halt.');
      } else {
        RewstDOM.showInfo('Debug simulation disabled.');
      }
    });
  }

  if (debugToggleLabel) {
    debugToggleLabel.style.display = isBetaMode() ? '' : 'none';
  }

  if (!debugVisibilityTimer) {
    debugVisibilityTimer = setInterval(() => {
      const label = commandDeck.querySelector('#prereq-debug-toggle-label');
      if (label) {
        const target = isBetaMode() ? '' : 'none';
        if (label.style.display !== target) label.style.display = target;
      }
    }, 1000);
  }
